...
Entity2D.prototype.isCollidedWith = function(other) {
  var pos={x:this.x+this.hitbox/2,y:this.y+this.hitbox/2};
  var otherpos={x:other.x+other.hitbox/2,y:other.y+other.hitbox/2};
  var minDistance = (this.hitbox/2 + other.hitbox/2);
  var d=(pos.x-otherpos.x)*(pos.x-otherpos.x)+(pos.y-otherpos.y)*(pos.y-otherpos.y);
  if (d<minDistance*minDistance)
    return true;
  else
  return false;
  //return (Util.getEuclideanDistance2(pos, otherpos) <=(minDistance * minDistance));
  //return (Util.getEuclideanDistance2(this.position, other.position) <=(minDistance * minDistance));
};
